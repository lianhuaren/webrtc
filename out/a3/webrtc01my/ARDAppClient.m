...
//  _settings = settings;
  ...
  if (kARDAppClientEnableTracing) {
    ...
    RTCLogError("===connectToRoomWithId");
  // Request TURN.
  __weak ARDAppClient *weakSelf = self;
//  [_turnClient requestServersWithCompletionHandler:^(NSArray *turnServers,
//                                                     NSError *error) {
//      RTCLogError("===requestServersWithCompletionHandler: %@", turnServers);
//    if (error) {
...
////      [strongSelf.iceServers addObjectsFromArray:@[@"stun:stun.xten.com"]];
...
//  }];
  ...
  [_roomServerClient joinRoomWithRoomId:roomId
      ...
      RTCLogError("===joinRoomWithRoomId: %@ error:%@", response, error);
    ARDAppClient *strongSelf = weakSelf;
    if (error) {
...
- (void)startSignalingIfReady {
  if (!_isTurnComplete || !self.clientId.length) {
    return;
  }
  ...
  if (_isInitiator) {
    ...
    [_peerConnection offerForConstraints:[self defaultOfferConstraints]
                       completionHandler:^(RTCSessionDescription *sdp,
                                           NSError *error) {
        RTCLogError("===offerForConstraints sdp: %@", sdp);
      ARDAppClient *strongSelf = weakSelf;
...
- (void)peerConnection:(RTCPeerConnection *)peerConnection
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    [self sendSignalingMessage:message];
      RTCLogError("===sendSignalingMessage messageType: %d", message.type);
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (!_isInitiator && !_peerConnection.localDescription) {
      ...
      [_peerConnection answerForConstraints:constraints
                          completionHandler:^(RTCSessionDescription *sdp,
                                              NSError *error) {
          RTCLogError("===answerForConstraints sdp: %@", sdp);
        ARDAppClient *strongSelf = weakSelf;
        [strongSelf peerConnection:strongSelf.peerConnection
            didCreateSessionDescription:sdp
                                  error:error];
